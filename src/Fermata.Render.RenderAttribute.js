(function () {
  ...
  Fermata.Render.prototype.AttributesClefProcess = [
    ...
    { key: 'clef-octave-change', type: _render.FuncTypes.$01, dataType: 'int' }
  ];
  ...
  Fermata.Render.prototype.AttributesKeys = function (node, i, attribut)
  {
    this.exploreSubNodes({ object: node, ctx: this,
      processes: typeof(node.fifths) !== 'undefined' ? _render.AttributesKeysTraditionalProcess : _render.AttributesKeysNonTraditionalProcess,
  ...
  {
    this.cur.measure.$fermata.attributes.partSymbol.symbol = typeof(node) === 'string' ? node : node.$t;
    if (node['top-staff']) {
