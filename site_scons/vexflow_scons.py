...
default_env = Environment(
    ...
    FERMATA_BUILD_PREFIX = "debug-4",
    FERMATA_VERSION = "0.0.1",
    FERMATA_BUILD_DATE = str(datetime.now()),
    JAVA = "java",
...
def fermata_stamper(target, source, env):
  """ A Build Stamper for Fermata """
  ...
  cmd += " -e s/__VEX_GIT_SHA1__/`cd deps/vexflow && git rev-list --max-count=1 HEAD`/ "
  cmd += " -e s/__FERMATA_BUILD_PREFIX__/$FERMATA_BUILD_PREFIX/"
  cmd += " -e s/__FERMATA_VERSION__/$FERMATA_VERSION/"
  cmd += ' -e "s/__FERMATA_BUILD_DATE__/${FERMATA_BUILD_DATE}/"'
  cmd += " -e s/__FERMATA_GIT_SHA1__/`git rev-list --max-count=1 HEAD`/ "
...
default_env.Append(
    BUILDERS = {'JavaScript': Builder(action = js_builder),
                'FermataStamp': Builder(action = fermata_stamper)})
...
def build_and_stamp(target, sources, env):
  ...
  final_node = env.FermataStamp(target, pre_node)
  return final_node
